...
fs.mkdir(config.releasesDir, function(err) {
    if (err && (err.code != 'EEXIST')) {
        console.log("Error creating logs directory, ("+config.releasesDir+ ") "+err.code)
    }
});
fs.mkdir(config.licenseDir, function(err) {
    if (err && (err.code != 'EEXIST')) {
        console.log("Error creating logs directory, ("+config.licenseDir+ ") "+err.code)
    }
});
fs.mkdir(config.syncDir, function(err) {
    if (err && (err.code != 'EEXIST')) {
        console.log("Error creating logs directory, ("+config.syncDir+ ") "+ err.code)
    }
});
fs.mkdir(config.thumbnailDir, function(err) {
    if (err && (err.code != 'EEXIST')) {
        console.log("Error creating logs directory, ("+config.thumbnailDir+") "+err.code)
    }
});
