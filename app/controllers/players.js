...
fs.mkdir(config.logStoreDir, function(err) {
    if (err && (err.code != 'EEXIST')) {
        console.log("Error creating logs directory, ("+config.logStoreDir+") " +err.code)
    }
});
...
exports.updateDisconnectEvent = function(socketId, reason) {
    Player.findOne({socket:socketId}, function(err,player) {
        if (!err && player) {
            ...
            console.log("disconnect:: player id: "+player._id+" | player name: "+player.name+";reason: "+reason)
        } else {
            console.log("not able to find player for disconnect event: "+socketId+"| err:",err);
        }
    })
}
...
var sendConfig = function (player, group, periodic) {
    ...
    retObj.authCredentials = settings.playerCredentials;
    ...
    Player.findOne({cpuSerialNumber: req.body.cpuSerialNumber}, function (err, data) {
        ...
        Group.findById(player.group._id, function(err,group) {
            if (!err && group) {
                ...
                console.log("[player.js:createObject] unable to find group (group id: " + player.group._id + ") for the player:: player => ", player);
            }
        })
        ...
        function (next) {
            ...
            Group.findById(object.group._id, function (err, group) {
                if (!err && group) {
                    ...
                    console.log("[player.js:updateObject] unable to find group (group id: " + player.group._id + ") for the player:: object => ",object);
                }
            })
    })
};
            ...
            if (!player.registered || obj.request ) {
                Group.findById(player.group._id, function (err, group) {
                    if (!err && group) {
                        ...
                        console.log("[player.js::updatePlayerStatus] unable to find group (group id :" + player.group._id + ") for the player (player serial: " + player.cpuSerialNumber + ")");
                    }
                })
            }
