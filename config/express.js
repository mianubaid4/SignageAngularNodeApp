...
var basicHttpAuth = function (req, res, next) {
    ...
    if (!auth) {     // No Authorization header was passed in so it's the first time the browser hit us
        ...
        require('../app/controllers/licenses').getSettingsModel(function (err, settings) {
            ...
            var cred = settings.authCredentials.find(x => x.user === username);
            if ((!username) ||
                (!cred.user || username == cred.user) &&
                (!cred.password || password == cred.password)) {
                //console.log("http request authorized for download for "+req.path);
                req.authCredentials = cred;
                next();
            } else {
                var ip = req.headers['x-forwarded-for'] || req.socket.remoteAddress
                console.log("Username:" + username + " IP: " + ip + " URL: " + req.originalUrl);
                console.log("http request rejected for download for " + "(User: " + cred.user + ") " + req.path);
...
module.exports = function (app) {
   ...
   // app.use(express.static('../dist/angularApp'))//set the static path 
